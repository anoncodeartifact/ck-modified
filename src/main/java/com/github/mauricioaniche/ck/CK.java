...
public class CK {
	...
	public void calculate(Path path, CKNotifier notifier, Path... javaFilePaths) {
		...
		long repoLoc = 0;
		long repoCommentLines = 0;

		for (String filePath : strJavaFilePaths) {
			try {
				String src = java.nio.file.Files.readString(java.nio.file.Paths.get(filePath));
				com.github.mauricioaniche.ck.util.LOCCalculator.LocAndComments lc =
						com.github.mauricioaniche.ck.util.LOCCalculator.calculateWithComments(src);
				repoLoc += lc.loc;
				repoCommentLines += lc.commentLines;
			} catch (Exception e) {
				log.warn("Failed to compute LOC/comments for file: " + filePath, e);
			}
		}

		notifier.notifyRepoSummary(repoLoc, repoCommentLines);

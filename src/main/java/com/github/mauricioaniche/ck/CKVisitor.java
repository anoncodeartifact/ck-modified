...
import com.github.mauricioaniche.ck.util.LOCCalculator.LocAndComments;
import static com.github.mauricioaniche.ck.util.LOCCalculator.calculateWithComments;
...
public class CKVisitor extends ASTVisitor {
	...
	public boolean visit(TypeDeclaration node) {
		...
		LocAndComments lc = calculateWithComments(node.toString());
		currentClass.setLoc(lc.loc);
		currentClass.setComment(lc.commentLines);
		currentClass.setStartLine(JDTUtils.getStartLine(cu, node));
		currentClass.setEndLine(JDTUtils.getEndLine(cu, node));
	...
	public boolean visit(MethodDeclaration node) {
		...
		LocAndComments lc = calculateWithComments(node.toString());
		currentMethod.setLoc(lc.loc);
		currentMethod.setComment(lc.commentLines);
		currentMethod.setStartLine(JDTUtils.getStartLine(cu, node));
		currentMethod.setEndLine(JDTUtils.getEndLine(cu, node));
